...
{
	nodo_error_t * nuevo_nodo, * ultimo_nodo;
	
	nuevo_nodo = (nodo_error_t *) malloc (sizeof(nodo_error_t));
	nuevo_nodo->info = error;
	nuevo_nodo->sig = NULL;
																					...
																					bool_t * hay_un_caracter_no_reconocido)
{
	...
	{	// SI NO ESTA VACIA REVISO SI CARACTER ANTERIOR ES NO RECONCIDO Y LO CONCATENO CON EL CARACTER DEL NODO SGTE
		if (*hay_un_caracter_no_reconocido == TRUE)
		{
			for (ultimo_nodo = *lista; ultimo_nodo->sig != NULL; ultimo_nodo = ultimo_nodo->sig)
				; 
			strcat (ultimo_nodo->info.cadena,error.cadena);
		}
...
{
	nodo_error_t * temporal;
	
	printf ("\n%-20s%-20s%s\n", "############### ", cabecera," ###############");
	for (temporal = lista; temporal != NULL; temporal = temporal->sig)
		 printf ("%s%-20s\t%-3s%d\n",etiqueta_cadena ,temporal->info.cadena, etiqueta_nro_linea, temporal->info.nro_linea);
...
void imprimir_palabra_reservada (nodo_error_t * lista)
{
	...
	printf ("\n%-20s%-20s%s\n", "############### ", "PALABRAS_RESERVADAS"," ###############");
	for (temporal = lista; temporal != NULL; temporal = temporal->sig)
		 printf ("%s%-35s%s%s\n","Palabra Reservada: " ,temporal->info.cadena, "Tipo: ", categorias[(int)temporal->info.nro_linea]);
...
void imprimir_comentarios (nodo_error_t * lista)
{
	...
	printf ("\n%-20s%-20s%s\n", "############### ", "COMENTARIOS"," ###############");
	for (temporal = lista; temporal != NULL; temporal = temporal->sig)
		 printf ("%s%-35s\n%s%s\n","Comentario: " ,temporal->info.cadena, "Tipo: ", categorias[(int)temporal->info.nro_linea]);
...
{
	nodo_error_t * nodo_actual = lista, * nodo_siguiente = NULL;
...
nodo_error_t * buscar (nodo_error_t * lista, error_t error)
{
	...
	for (temporal = lista; temporal != NULL && strcmp(temporal->info.cadena,error.cadena) != 0; temporal = temporal->sig)
		;	
	return temporal;
}
...
{
	nodo_error_t * temporal = buscar (*lista,error);
	
	if (temporal == NULL)
		agregar_error_al_final_con_repeticion (lista,error);
	else
		(*accion)(temporal);
...
{
	return dato1.nro_linea - dato2.nro_linea;
}
...
void agregar_uno_a_entero (nodo_error_t * nodo)
{
	nodo->info.nro_linea = nodo->info.nro_linea + 1;
	return;
}
// UNA FUNCION QUE PUEDE SER UNA ACCION Y QUE SE USA EN LA FUNCION "AGREGAR SIN REPETICION"
...
void agregar_ordenado (nodo_error_t ** lista, error_t dato,
							int (*criterio)(error_t dato1,error_t dato2))
{
	nodo_error_t * anterior = NULL, * actual = *lista;
	...
	if (actual != *lista)
	{
		anterior->sig = (nodo_error_t *) malloc (sizeof (nodo_error_t));
		anterior->sig->info = dato;
		anterior->sig->sig = actual;
	}
	else
	{
		*lista = (nodo_error_t *) malloc (sizeof (nodo_error_t));
		(*lista)->info = dato;
		(*lista)->sig = actual;
	}
...
void agregar_error_ordenado_sin_repeticion (nodo_error_t ** lista, error_t error, 
																	...
																	int (*criterio)(error_t dato1,error_t dato2))
{
	...
	if (temporal == NULL)
		agregar_ordenado (lista,error,criterio);
	else
		(*accion)(temporal);
